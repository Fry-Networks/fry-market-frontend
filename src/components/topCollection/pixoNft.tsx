...
const PixoNft = ({ nfts, collectionData, auctionedNfts, allBoughtNft, loadingBought, loadingListed, loadingAuctioned }: any) => {
  ...
  const filteredCards = trendingCard.filter((card) => card.userName.toLowerCase().startsWith(searchTerm.toLowerCase()))
                        ...
                        {allBoughtNft.length > 0 ? (
                          <div className="popularcardContainer grid grid-cols-4 gap-8 mt-5">
                            {allBoughtNft.map((data: any, index: any) =>
                              searchTerm ? (
                                data.name.toUpperCase().includes(searchTerm.toUpperCase()) ? (
                                  <CollectionsCard
                                    ...
                                    label={activeAccount?.address ? 'List' : 'List'}
                                    collectionData={collectionData}
                                  />
                                ) : index == allBoughtNft.length - 1 ? (
                                  allBoughtNft.filter((data: any) => data.name.toUpperCase().includes(searchTerm.toUpperCase())).length >
                                    0 ? (
                                    ''
                                  ) : (
                                    'No Result Found'
                          ...
                          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>No NFTs Owned Yet.</div>
                        )}
                      </>
                    )}
                  </Tabs.TabPane>
                              ...
                              ? nfts.map((data: any, index: any) => {
                                ...
                                return searchTerm ? (
                                  data.name.toUpperCase().includes(searchTerm.toUpperCase()) ? (
                                    ...
                                    nfts.filter((data: any) => data.name.toUpperCase().includes(searchTerm.toUpperCase())).length > 0 ? (
                                      ''
                                    ) : (
                                      'No Result Found'
                                    )
                            ...
                            {auctionedNfts.map((data: any, index: any) =>
                              searchTerm ? (
                                data.name.toUpperCase().includes(searchTerm.toUpperCase()) ? (
                                  ...
                                  />
                                ) : index == auctionedNfts.length - 1 ? (
                                  auctionedNfts.filter((data: any) => data.name.toUpperCase().includes(searchTerm.toUpperCase())).length >
                                    0 ? (
                                    ''
                                  ) : (
                                    'No Result Found'
