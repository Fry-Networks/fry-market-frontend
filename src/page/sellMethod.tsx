  ...
  const handleAuctionNft = async () => {
    return new Promise(async (resolve, reject) => {
      try {
        if (activeAccount?.address) {
          setLoading(true)
          const response: any = await listNftAuction(
            activeAccount?.address,
            signer,
            nftData.nftAddress,
            price * 1000000,
            minimumBidAmount * 1000000,
            biddingDuration.biddingStartTime,
            biddingDuration.biddingEndTime,
          )
  ...
  const handleSubmit = () => {
    if (price) {
      if (selectedMethod == 'fixed') {
        toast.promise(handleListNft(), {
          pending: 'NFT is listing',
        ...
        if (biddingDuration.biddingStartTime && biddingDuration.biddingEndTime && minimumBidAmount) {
          if (biddingDuration.biddingStartTime > Math.floor(currentDate.getTime() / 1000)) {
            if (biddingDuration.biddingEndTime > biddingDuration.biddingStartTime) {
              toast.promise(handleAuctionNft(), {
                pending: 'NFT is listing on auction',
        ...
        <div className="container">
          <div className="inner my-20 flex gap-[20px] h-full">
            <div className="leftArea w-1/4 flex flex-col justify-start gap-5">
              ...
              />
              <img
                className="  border-solid border-[20px] border-[white] rounded-3xl "
                src={nftData?.url ? replaceJsonWithPng(nftData?.url) : sellImg}
                alt=""
            ...
            >
              ...
              <div className="mt-6">
                ...
                <div className="selectMethodContainer flex gap-7 mt-6">
                  <div
                    ...
                    style={{
                      boxShadow: '2.809px 2.809px 10.534px 0px rgba(0, 0, 0, 0.20)',
                    }}
                  >
              ...
              <div className="flex flex-col gap-1 mt-3 priceBox rounded-xl">
                <label className="darkBlack large font-medium font-Roboto">{selectedMethod == 'fixed' ? 'Price' : 'Start Amount'}</label>
              ...
              {selectedMethod === 'auction' && (
                <>
                  <div className="flex flex-col gap-1 mt-3 ">
                    <label className="darkBlack large font-medium font-Roboto">Minimum Bid Amount</label>
                    <div className="flex justify-start items-center gap-3 enterPrice">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          setMinimumBidAmount(e.target.value)
                        }}
                      />
                    </div>
                  </div>
                  <div className="mt-3 flex flex-col gap-2 duration priceBox rounded-xl">
                    <label className="darkBlack large font-medium font-Roboto">Bidding Start Time</label>
                  ...
                  <div className="mt-3 flex flex-col gap-2 duration">
                    <label className="darkBlack large font-medium font-Roboto">Bidding End Time</label>
