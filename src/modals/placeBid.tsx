...
const PlaceBid = ({ isbidmodal, setisbidmodal, data, getAuctionedNft }: any) => {
  ...
  return (
    <>
      <Modal open={isbidmodal} onOk={handleOk} onCancel={handleCancel} centered={true} width={415} footer={null}>
        <div style={{ zIndex: '9999' }} className="connectModal z-[9999]">
          <div className="w-full">
            <p className="fw-bold ex-large font-Apex font-normal darkBlack text-center">Place your bid</p>
          </div>
          <div className="innerContent flex flex-col items-center gap-5 mt-4 ">
            ...
            <div className="serviceDiv w-full flex justify-between items-center">
              ...
              <p className="darkBlack font-Roboto ex-large font-medium">
                {(Number(bidAmount) + Number(bidAmount) * (data.totalListcount == 1 ? 0.3 : 0.1)).toFixed(2)} FRY
              </p>
            </div>
            <img src={redline} alt="" />
                ...
                onClick={async () => {
                  if (activeAccount?.address) {
                    if (Number(bidAmount) > Number(data.highestBidAmount / 1000000 + data.minBidAmount / 1000000)) {
                      ...
                      toast.error(
                        `Minimum Bidding Amount should be more than ${Number(data.highestBidAmount / 1000000 + data.minBidAmount / 1000000).toFixed(2)}`,
                      )
                    }
                  } else {
